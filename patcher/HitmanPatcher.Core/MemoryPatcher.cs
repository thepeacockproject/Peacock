...
public static class MemoryPatcher
{
    ...
    public static void ClearPatchedProcesses()
    {
    ...
    public static void PatchAllProcesses(ILoggingProvider logger, Options patchOptions)
    {
        ...
        {
            if (patchedprocesses.Add(process.Id))
            {
                try
                {
                    ...
                    catch (Win32Exception ex)
                    {
                        ...
                        {
                            logger.log("Access denied, try running the patcher as admin.");
                            process.Dispose();
                            continue;
                        }

                        // The process has exited already
                        dontPatch = true;
                    }
    ...
    private static bool IsReadyForPatching(IntPtr hProcess, IntPtr baseAddress, HitmanVersion version)
    {
        ...
        {
            if (!Pinvoke.VirtualProtectEx(hProcess, baseAddress + p.offset, (UIntPtr)1, newMemProtection, out var oldProtectFlags))
            {
                throw new Win32Exception(Marshal.GetLastWin32Error(), string.Format("error at vpe1Check for offset {0:X}", p.offset));
            }
            if (!Pinvoke.ReadProcessMemory(hProcess, baseAddress + p.offset, buffer, (UIntPtr)1, out _))
            {
    ...
    public static UInt32 GetTimestamp(IntPtr hProcess, IntPtr baseAddress)
    {
        ...
        Pinvoke.ReadProcessMemory(hProcess, baseAddress + 0x3C, buffer, (UIntPtr)4, out _);
        int ntHeaderOffset = BitConverter.ToInt32(buffer, 0);
        Pinvoke.ReadProcessMemory(hProcess, baseAddress + ntHeaderOffset + 0x8, buffer, (UIntPtr)4, out _);
